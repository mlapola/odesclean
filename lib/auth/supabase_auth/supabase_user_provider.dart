...
class OdescleanZapSupabaseUser extends BaseAuthUser {
  OdescleanZapSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> odescleanZapSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = OdescleanZapSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
