...
class OdescleanWhatsappSupabaseUser extends BaseAuthUser {
  OdescleanWhatsappSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> odescleanWhatsappSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = OdescleanWhatsappSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
